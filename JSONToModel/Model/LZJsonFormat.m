    ...
    if ([ESJsonFormatSetting defaultSetting].outputToFiles) {
        ...
        if (!self.isSwift) {
            ...
            if ([ESJsonFormatSetting defaultSetting].impOjbClassInArray) {
                NSString *methodStr = [LZJsonFormatManager methodContentOfObjectClassInArrayWithClassInfo:classInfo];
                if (methodStr.length) {
...
- (void)didApplicationFinishLaunchingNotification:(NSNotification*)noti{
    ...
    if (menuItem) {
        ...
        NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:@"LZJsonFormat" action:nil keyEquivalent:@""];
        item.submenu = menu;
